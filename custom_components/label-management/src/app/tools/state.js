...
let currChangeState = 'init';

function getTasksChangeState() {
  return currChangeState;
}
...
function setTasksChangeState(value) {
  currChangeState = value;
  const originalTitle = document.title;
  if (currChangeState === 'changed') {
    if (!document.title.startsWith('* ')) {
      document.title = `* ${originalTitle}`;
    }
  } else if (currChangeState === 'init') {
    if (document.title.startsWith('* ')) {
      document.title = originalTitle;
    }
  }
