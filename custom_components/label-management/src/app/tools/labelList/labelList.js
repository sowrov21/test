...
import { updateImageState } from '../imageList/imageList';
...
async function createNewDropdown() {
  const labelDropdownOptions = await getLabelOptions();
  let dropdown = '<tbody>';
...
function repopulateDropdown() {
  ...
  // }
  (async () => { // new add by sowrov
    const dropdown = await createNewDropdown();
    const dropdownParentElements = document.getElementsByClassName('dropdown-content');
    for (let i = 0; i < dropdownParentElements.length; i += 1) {
      dropdownParentElements[i].innerHTML = dropdown;
    }
...
async function deleteAndAddLastRowToRefreshDropdownDiv(dropdownLabelsElement) {
...
async function initLabelEditing(id) {
  ...
  availableListOptions = await getLabelOptions();
...
function finishEditingLabelList(event) {
  if (isEditingLabel) {
    if (event.target.matches('.labelDropdownOption')) {
      ...
      setTasksChangeState('changed');
      updateImageState(document.getElementById('image-name').innerText);
