...
let labelOptions = defaultLabelOptions;
...
function generateRandomColorHue() {
  return Math.floor(Math.random() * 360); // 0–359 degrees hue
}
async function getLabelOptions() {
  ...
  const localOptions = localStorage.getItem('labelOptions');
  if (localOptions && localOptions !== 'undefined' && localOptions !== '') {
    labelOptions = JSON.parse(localOptions);
    return labelOptions;
  }
  const payload = {
    skip: 0,
  ...
  const response = await APIClient('products/getProducts', payload, 'POST', token);
  const labelItems = response.data.items || [];
  // console.log('Response from getLabelOptions:', response);
  if (!response.success || !Array.isArray(labelItems)) {
    return defaultLabelOptions; // fallback
  }

  const labelO = labelItems.map((product, index) => {
    const hue = generateRandomColorHue();
    return {
      text: product.product_name, // product.product_name
      client_id: product.client_id,
      id: product.id,
      default: index === 0, // first item default
      color: {
        default: `hsl(${hue}, 93%, 45%, 0.01)`,
        highlight: `hsl(${hue}, 93%, 45%, 0.3)`,
        stroke: `hsl(${hue}, 93%, 45%)`,
        label: `hsl(${hue}, 93%, 45%, 0.25)`,
      },
    };
  });
  // console.log('Label Options:', labelO);
  labelOptions = labelO.length > 0 ? labelO : defaultLabelOptions;
  localStorage.setItem('labelOptions', JSON.stringify(labelOptions));
  return labelOptions;
}
