...
import {
  ...
  getTasksChangeState, setTasksChangeState,
} from '../state';
...
function updateCurrentImageIdsNew(currentId, newId) {
  currentlySelectedImageId = currentId;
  newImageId = newId;
}
...
function updateImageState(imgName) {
  const id = getImageIdByName(imgName);
  if (getTasksChangeState() === 'init') { // on first time load
    images[id].hasChanged = false;
  } else if (getTasksChangeState() === 'changed') {
    images[id].hasChanged = true;
  } // else if (getTasksChangeState() === 'image_switch') { // do no changes to image during switch
  // images[id].hasChanged = true; // no chnage
  // }
}
function resetAllImageStatus() {
  for (let i = 0; i < images.length; i += 1) {
    if (images[i].hasChanged) {
      images[i].hasChanged = false;
    }
  }
  setTasksChangeState('changed');
}
...
function displayTickSVGOverImageThumbnail(id) {
  ...
  if (image && image.thumbnailElementRef && image.thumbnailElementRef.childNodes[2]) {
    image.thumbnailElementRef.childNodes[2].style.display = 'block';
    image.isLabeled = true;// Mark labeled true
  }
}
...
function removeTickSVGOverImageThumbnail(id) {
  ...
  if (image && image.thumbnailElementRef && image.thumbnailElementRef.childNodes[2]) {
    image.thumbnailElementRef.childNodes[2].style.display = 'none';
    image.isLabeled = false;// Mark labeled false
  }
}
...
function addNewImage(imageName, imageData, url = '', imgId = null, taskItemId = null) {
...
function addSingleImageToList(imageMetadata, imageData, url = '', imgId = null, taskItemId = null) {
  addNewImage(imageMetadata.name, imageData, url, imgId, taskItemId);
  setDefaultImageProperties(images[newImageId], imageMetadata);
  displayUploadedImage(imageMetadata, false);
  newImageId += 1;
}

function addImageFromMultiUploadToList(imageMetadata, imageData, isfirstFromMany, url = '', imgId = null, taskItemId = null) {
  addNewImage(imageMetadata.name, imageData, url, imgId, taskItemId);
  setDefaultImageProperties(images[newImageId], imageMetadata);
  // setDefaultImageProperties(images[imgId], imageMetadata);
...
function disableAllCanvasObjects() { // add by soworov
  if (!canvas) return;
...
function switchImage(direction) {
  setTasksChangeState('image_switch');
...
// new added by sowrov on 31 July 2025
function removeAllImages() {
  // Remove all image thumbnail DOM elements
...
export {
  ...
  removeMLThumbnailHighlight, removeSelectedMLThumbnailHighlight, getImageIdByName, removeAllImages,
  updateCurrentImageIdsNew, updateImageState, resetAllImageStatus,
};
