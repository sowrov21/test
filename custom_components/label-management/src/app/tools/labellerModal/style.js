...
async function deleteAndAddLastRowToRefreshDiv() {
...
async function initialiseLabellerModalOptionsList() {
  setLocalVariables();
  const labelOptions = await getLabelOptions();
...
// Handle input changes
    async function handleInput() {
        ...
        const labelOptions = await getLabelOptions();
        if (searchTerm === '') {
          //const top10 = await getLabelOptions().slice(0, 10);
          const top10 = labelOptions.slice(0, 10);
            renderDropdownItems(top10);
            addNewItem.style.display = 'none';
            return;
        }

        // const filteredItems = await getLabelOptions().filter((item) =>
        //     item.text.toLowerCase().includes(searchTerm),
        // );
        const filteredItems = labelOptions.filter((item) =>
            item.text.toLowerCase().includes(searchTerm),
        );
...
async function resetLabellerModalOptions() {
