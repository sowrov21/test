...
import {
  ...
  getContinuousDrawingState, getCrosshairUsedOnCanvasState, setTasksChangeState,
} from '../state';
    ...
    window.setTimeout(async () => {
      ...
      for (let i = 0; i < optionsElementList.length; i += 1) {
        if (optionsElementList[i].childNodes[0].childNodes[0].childNodes[0].innerHTML
          ...
          currentlySelectedLabelOption.style.backgroundColor = await getLabelOptions()[i].color.label;
